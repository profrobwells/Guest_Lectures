...
summary(Data$Salary)
#The median salary for a person with data responsibilities is $59,000 versus $50,625 for someone named as reporter.
```
...
options(scipen=999)
ggplot(data=Biz) +
  geom_col(mapping=aes(x=Salary, y=COMPANY)) 
...
```{r}
ggplot(data=Biz) +
  geom_col(mapping=aes(x=Salary, y=COMPANY, fill=Salary))
...
ggplot(Biz,aes(x = Salary, y = COMPANY,
             fill = Salary)) +
...
#This is your title sequence
  labs(title = "Salaries for `Business` Publications",
       subtitle = "Crowdsourced Media Salary Details",
       caption = "Graphic by Rob Wells, 3-8-2024",
       y="Publication",
       x="Salary")
```
...
MediaBucks %>% 
  filter(COMPANY == "WallStreetJournal") %>% 
#Sandwich it onto a simple ggplot
  ggplot(aes(x = Salary, y = Race, fill = Race)) +
  geom_col(position = "dodge") + 
  theme(legend.position = "none") +
  labs(title = "Salaries of Wall Street Journal Staff by Race", 
       subtitle = "Crowdsourced Media Salaries Data",
